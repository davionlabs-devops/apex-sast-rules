...
module ApexPyDeserCfg implements DataFlow::ConfigSig {
  ...
  predicate isSink(DataFlow::Node sink) {
    sink = API::moduleImport("pickle").getMember("loads").getACall().getArg(0)
    or
    sink = API::moduleImport("pickle").getMember("load").getACall().getArg(0)
    or
    sink = API::moduleImport("_pickle").getMember("loads").getACall().getArg(0)
    or
    sink = API::moduleImport("cPickle").getMember("loads").getACall().getArg(0)
    or
    sink = API::moduleImport("yaml").getMember("load").getACall().getArg(0)
  }
}
